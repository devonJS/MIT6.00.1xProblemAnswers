...
def isValidWord(word, hand, wordList):
    ...
    valid_hand = hand.copy()
    
    for i in range(len(word)):
        if valid_hand.get(word[i], 0) == 0:
            return False
        else:
            valid_hand[word[i]] -= 1
    ...
    while calculateHandlen(hand) > 0:
        ...
        if user_input == '.':
            ...
            if not isValidWord(user_input, hand, wordList):
                ...
                add_score = getWordScore(user_input, HAND_SIZE)
                total_score += add_score
                print '"' + user_input + '"' + " earned " + str(add_score) + " points. Total: " + str(total_score) + " points"
...
def playGame(wordList):
    ...
    """
    option_input = raw_input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")
